...
if (
    grp not in adata.obs.columns
    and grp == "fibrosis_bin"
    and "fibrosis_axis" in adata.obs
):
    adata.obs["fibrosis_bin"] = np.where(
        adata.obs["fibrosis_axis"].astype(float) >= f2plus, "high", "low"
    )
...
emb = (
    ...
    else ("X_emb" if "X_emb" in adata.obsm else None)
)
...
for comp in compartments:
    ...
    if emb is not None:
        ...
        sc.pp.pca(
            sub, n_comps=min(30, sub.n_obs - 1, sub.n_vars - 1), random_state=seed
        )
    ...
    per_donor = (
        (d.groupby(["donor_id", "subcluster"]).size() / d.groupby("donor_id").size())
        .rename("frac")
        .reset_index()
    )
    ...
    for sc_id, g in per_donor.groupby("subcluster"):
        ...
        summary_rows.append(
            dict(
                ...
                subcluster=sc_id,
                n_cells=n_cells,
                frac_test=round(f_tst, 4),
                frac_ref=round(f_ref, 4),
                disease_log2fc=round(log2fc, 3),
                disease_associated=assoc,
            )
        )
    ...
    sc.tl.rank_genes_groups(
        sub, "subcluster", method="wilcoxon", n_genes=300, use_raw=False
    )
...
if not markers.empty:
    ...
    best = (
        markers.sort_values("abs_score", ascending=False)
...
log.info(
    f"niche: {n_assoc} disease-associated subclusters; "
