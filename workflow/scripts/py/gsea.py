...
def empty(note):
    pd.DataFrame(
        columns=["Term", "ES", "NES", "pval", "fdr", "lead_genes", "status"]
    ).assign(status=note).to_csv(out, sep="\t", index=False)
...
try:
    ...
    keep = [
        c
        for c in ["Term", "ES", "NES", "pval", "fdr", "lead_genes"]
        if c in res.columns
    ]
    res = res[keep]
    res["status"] = "ok"
