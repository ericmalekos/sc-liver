...
def aggregate(estimate_df, level):
    ...
    long = grouped.reset_index().melt(id_vars=group, var_name="source", value_name="mean_activity")
    long["level"] = level
    return long
...
try:
    ...
    dc.run_mlm(mat=adata, net=prog, source="source", target="target", weight="weight",
...
if bool(p.collectri):
    try:
        ...
        dc.run_ulm(mat=adata, net=net, source="source", target="target", weight="weight",
                   verbose=False, use_raw=False)
...
try:
    dc.run_aucell(mat=adata, net=geneset_long, source="source", target="target", verbose=False, use_raw=False)
    ...
    for s in geneset_long["source"].unique():
        genes = [g for g in geneset_long.loc[geneset_long.source == s, "target"] if g in adata.var_names]
        if genes:
...
activity = pd.concat(records, ignore_index=True) if records else pd.DataFrame(
    columns=group + ["source", "mean_activity", "level"])
...
try:
    src = activity[activity.level == "pathway_progeny"] if not activity.empty else activity
    if src.empty:
        src = scores
    piv = src.pivot_table(index="compartment", columns="source", values="mean_activity", aggfunc="mean")
    fig, ax = plt.subplots(figsize=(min(12, 1 + 0.5 * piv.shape[1]), 1 + 0.4 * piv.shape[0]))
    ...
    fig.tight_layout(); fig.savefig(snakemake.output.fig, dpi=110)  # noqa: F821
except Exception as e:
    log.warning(f"activity figure fallback: {e}")
    plt.figure(); plt.text(0.5, 0.5, "no pathway activity", ha="center")
