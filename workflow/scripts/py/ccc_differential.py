...
if liana.empty or "specificity_rank" not in liana:
    pd.DataFrame(columns=diff_cols).to_csv(
        snakemake.output.diff, sep="\t", index=False
    )  # noqa: F821
    pd.DataFrame(columns=nn_cols).to_csv(
        snakemake.output.nichenet, sep="\t", index=False
    )  # noqa: F821
...
liana = liana[
    liana["specificity_rank"] <= max(spec_cut, liana["specificity_rank"].min())
]
...
fibrotic = next(
    (c for c in conds if str(c).lower() in ("cirrhotic", "high", "mash", "fibrotic")),
...
if fibrotic is not None and liana["condition"].nunique() >= 2:
    piv = liana.pivot_table(
        index=key, columns="condition", values="score", aggfunc="mean"
    ).fillna(0)
...
diff[[c for c in diff_cols if c in diff.columns]].to_csv(
    snakemake.output.diff, sep="\t", index=False
)  # noqa: F821
...
rows = []
fib_diff = (
    diff[diff.get("enriched_in_fibrosis", True)]
    if "enriched_in_fibrosis" in diff
    else diff
)
for _, r in fib_diff.iterrows():
    ...
    rows.append(
        dict(
            ...
            n_targets_up=len(hit),
            frac_targets_up=round(len(hit) / len(targets), 3),
            corroborated=len(hit) >= min_targets,
            targets_up=",".join(hit),
        )
    )
nn = (
    pd.DataFrame(rows, columns=nn_cols)
    .drop_duplicates()
    .sort_values("n_targets_up", ascending=False)
    if rows
    else pd.DataFrame(columns=nn_cols)
)
nn.to_csv(snakemake.output.nichenet, sep="\t", index=False)  # noqa: F821
log.info(
    f"differential CCC: {len(diff)} interactions; corroborated ligand-receiver links: "
