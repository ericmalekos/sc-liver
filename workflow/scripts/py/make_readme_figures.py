...
_scatter_cat(
    axes[0], ann.obs["sample_id"], "By sample — batches intermix", TAB20, legend=False
)
_scatter_cat(
    axes[1],
    ann.obs["condition"],
    "By condition — preserved",
    ["#d73027", "#4575b4"],
    legend=True,
...
rows = [
    ("batch_silhouette", "batch silhouette  (→0 mixed)", "#888888"),
...
_scatter_cat(
    ax,
    ann.obs["compartment"],
    "Cell compartments — primary cohort",
    TAB10,
    legend=True,
...
de = pd.read_csv(
    f"results/05_de/{PRIMARY}/stellate_myofibroblast/de_results.tsv", sep="\t"
)
...
for _, r in de[up].nlargest(8, "nlp").iterrows():
    ax.annotate(
        r["gene"], (r["log2FoldChange"], r["nlp"]), fontsize=7, ha="left", va="bottom"
    )
ax.axvline(0.58, ls="--", lw=0.5, color="grey")
ax.axvline(-0.58, ls="--", lw=0.5, color="grey")
...
ax.set_xticklabels(paths, rotation=45, ha="right", fontsize=8)
ax.set_yticks(range(len(comps6)))
ax.set_yticklabels(comps6, fontsize=8)
cb = plt.colorbar(im, ax=ax, fraction=0.02, pad=0.02)
cb.set_label("activity shift\n(cirrhotic - healthy)", fontsize=7.5)
cb.ax.tick_params(labelsize=7)
ax.set_title(
    "PROGENy pathway-activity shift in fibrosis (decoupler), per compartment",
    fontsize=10,
)
...
ax.axline(
    (0, 0), (1, 1), color="#bbbbbb", lw=0.9, ls="--", zorder=0
)  # y = x (perfect agreement)
ax.axhline(0, color="#e8e8e8", lw=0.6)
ax.axvline(0, color="#e8e8e8", lw=0.6)
handles7 = []
for c in comps7:
    ...
    rho = (
        spearmanr(d["primary_lfc"], d["valid_lfc"], nan_policy="omit").correlation
        if len(d) >= 5
        else float("nan")
    )
    handles7.append(
...
ax.legend(
    handles=handles7,
    title="compartment (Spearman rho)",
    fontsize=7,
    title_fontsize=7.5,
    frameon=False,
    loc="upper left",
...
ax.barh(
    range(len(sel)),
    sel["composite"],
    color=[cmap[c] for c in sel["compartment"]],
    height=0.72,
)
ax.set_yticks(range(len(sel)))
ax.set_yticklabels(sel["gene"], fontsize=8)
ax.set_xlabel("composite score", fontsize=9)
ax.set_title("Top prioritized biomarker / target candidates", fontsize=11)
ax.tick_params(labelsize=8)
handles = [
    Line2D([0], [0], marker="s", linestyle="", markersize=8, color=cmap[c])
    for c in comps
]
ax.legend(
    handles,
